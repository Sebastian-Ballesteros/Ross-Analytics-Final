...
"data_train" and "data_test" datasets are read in:

The "data_train" dataset is upsampled using the upSample function from the caret library to address class imbalance. The minority class, represented by attrition_yes, is oversampled to balance the dataset.
...
data_train_up <- upSample(data_train[, !names(data_train) %in% "attrition_yes"],as.factor(data_train$attrition_yes), yname = "attrition_yes")
...
```{r}
DT_model <- C5.0(as.factor(attrition_yes) ~ ., data = data_train)
```
...
```{r}
DT_pred <- as.factor(predict(DT_model, data_test))

confusionMatrix(DT_pred,as.factor(data_test$attrition_yes), positive = "1")
...
```{r}
DT_model_boost <- C5.0(as.factor(attrition_yes) ~ ., data = data_train, trials = 10)

DT_boost_pred <- as.factor(predict(DT_model_boost, data_test))

confusionMatrix(DT_boost_pred,as.factor(data_test$attrition_yes), positive = "1")
...
```{r}
set.seed(12345)
RF_model <- randomForest(as.factor(attrition_yes)~.,data = data_train_up, mtry = 5)

RF_pred <- as.numeric(predict(RF_model, data_test))-1

confusionMatrix(as.factor(RF_pred), as.factor(data_test$attrition_yes))
